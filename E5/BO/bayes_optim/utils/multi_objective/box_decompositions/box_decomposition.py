...
class BoxDecomposition(Module, ABC):
    ...
    def __init__(self, ref_point: Tensor, sort: bool, Y: Optional[Tensor] = None) -> None:
    ...
    def pareto_Y(self) -> Tensor:
        ...
        try:
            ...
            raise ValueError("pareto_Y has not been initialized")
    ...
    def _reset_pareto_Y(self) -> bool:
        ...
        if not hasattr(self, "_neg_pareto_Y") or not torch.equal(pareto_Y, self._neg_pareto_Y):
...
class FastPartitioning(BoxDecomposition, ABC):
    ...
    def _partition_space(self):
        ...
        if len(self.batch_shape) > 0:
            ...
            raise NotImplementedError("_partition_space does not support batch dimensions.")
        # this assumes minimization
