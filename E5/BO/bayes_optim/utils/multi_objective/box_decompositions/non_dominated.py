...
from .box_decomposition import BoxDecomposition, FastPartitioning
from .utils import (
    _expand_ref_point,
...
class NondominatedPartitioning(BoxDecomposition):
    ...
    def _partition_space(self) -> None:
        ...
        cell = torch.zeros(2, self.num_outcomes, dtype=torch.long, device=self._neg_Y.device)
        ...
        self.register_buffer(
            "hypercells",
            torch.empty(2, 0, self.num_outcomes, dtype=torch.long, device=self._neg_Y.device),
        )
        outcome_idxr = torch.arange(self.num_outcomes, dtype=torch.long, device=self._neg_Y.device)
        ...
        if self._neg_pareto_Y.shape[-2] == 0:
            ...
            # `n_pareto + 2 x m`
            aug_pareto_Y = torch.cat([ideal_point, self._neg_pareto_Y, anti_ideal_point], dim=0)
            ...
            while len(stack) > 0:
                ...
                cell = stack.pop()
                cell_bounds_pareto_idcs = aug_pareto_Y_idcs[cell, outcome_idxr]
                cell_bounds_pareto_values = aug_pareto_Y[cell_bounds_pareto_idcs, outcome_idxr]
                # Check cell bounds
                ...
                if (cell_bounds_pareto_values[1] <= self._neg_pareto_Y).any(dim=1).all():
                    # Cell is entirely non-dominated
                    self.hypercells = torch.cat(
                        [self.hypercells, cell_bounds_pareto_idcs.unsqueeze(1)], dim=1
                    )
                elif (cell_bounds_pareto_values[0] <= self._neg_pareto_Y).any(dim=1).all():
    ...
    def get_hypercell_bounds(self) -> Tensor:
        ...
        """
        ref_point = _expand_ref_point(ref_point=self.ref_point, batch_shape=self.batch_shape)
    ...
    def _get_hypercell_bounds(self, aug_pareto_Y: Tensor) -> Tensor:
        ...
        if len(self.batch_shape) > 0:
            ...
            batch_idxr = (
                torch.arange(self.batch_shape[0], dtype=torch.long, device=self._neg_Y.device)
                .unsqueeze(1)
    ...
    def compute_hypervolume(self) -> Tensor:
        ...
        non_dom_volume = (cell_bounds_values[1] - cell_bounds_values[0]).prod(dim=-1).sum(dim=-1)
...
class FastNondominatedPartitioning(FastPartitioning):
    ...
    def _get_partitioning(self) -> None:
        ...
        self._Z2 = self.ref_point.expand(1, self.num_outcomes, self.num_outcomes).clone()
    ...
    def compute_hypervolume(self):
        ...
        total_volume = (ideal_point.squeeze(-2) - self.ref_point).clamp_min(0.0).prod(dim=-1)
        finite_cell_bounds = torch.min(self.hypercell_bounds, ideal_point)
        non_dom_volume = (
            (finite_cell_bounds[1] - finite_cell_bounds[0]).clamp_min(0.0).prod(dim=-1).sum(dim=-1)
        )
        return total_volume - non_dom_volume
