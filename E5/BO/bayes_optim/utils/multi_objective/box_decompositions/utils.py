...
def _expand_ref_point(ref_point: Tensor, batch_shape: Size) -> Tensor:
    ...
    if ref_point.shape[:-1] != batch_shape:
        ...
        ref_point = ref_point.view(*(1 for _ in batch_shape), ref_point.shape[-1]).expand(
            batch_shape + ref_point.shape[-1:]
        )
    return ref_point
...
def compute_local_upper_bounds(U: Tensor, Z: Tensor, z: Tensor) -> Tuple[Tensor, Tensor]:
...
def compute_dominated_hypercell_bounds_2d(pareto_Y_sorted: Tensor, ref_point: Tensor) -> Tensor:
