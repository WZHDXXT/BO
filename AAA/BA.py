...
,
    ...
    tr_lb = torch.clamp(x_center - weights * state.length, -5.0, 5.0)
    tr_ub = torch.clamp(x_center + weights * state.length, -5.0, 5.0)
...
def embedding_matrix(input_dim: int, target_dim: int, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    dtype=torch.float32
) -> torch.Tensor:
    if (
...
,
    ...
    # Compute the covariance matrix (Shape: (input_dim, input_dim))
...
,
    ...
    # Compute the RBF kernel matrix (kernel computation is for high-dimensional space)
    kernel_matrix = torch.tensor(rbf_kernel(data.cpu().numpy(), gamma=gamma), device=device, dtype=dtype)
    
    # Center the kernel matrix (Kernel PCA centering)
    ...
    # Eigen-decomposition (eigenvectors and eigenvalues of the centered kernel matrix)
    ...
    # Now we need to generate the embedding matrix for mapping low-dimensional data back to high-dimensional space
    embedding_matrix = torch.zeros((target_dim, input_dim), dtype=dtype, device=device)

    # Compute the projection of the top eigenvectors onto the input space (i.e., the high-dimensional space)
    for i in range(target_dim):
        eigenvector_column = top_eigenvectors[:, i].unsqueeze(1)  # Shape: (n_samples, 1)        
        # Compute projection of the eigenvector on the original data (mapping to high-dimensional space)
    ...
    # Return the embedding matrix with shape (target_dim, input_dim)
    return embedding_matrix
...
def increase_embedding_and_observations(
    S: torch.Tensor, X: torch.Tensor, n_new_bins: int, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    dtype=torch.float32
,
) -> torch.Tensor:
...
def multi_scale_embedding(input_dim: int, target_dim: int, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    dtype=torch.float32
) -> torch.Tensor:
...
def increase_multi_scale_embedding_and_observations(
    S: torch.Tensor, X: torch.Tensor, n_new_bins: int, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    dtype=torch.float32
    ...
    # Current dimensions
    target_dim, input_dim = S.shape
    n_samples = X.size(0)

    # Clone the current matrices for updates
    S_update = S.clone()
    X_update = X.clone()

    # Calculate scales and step size for the embedding matrix
    scales = target_dim  # Assumes each target dimension corresponds to a scale
    step = input_dim // scales

    # Add new rows to the embedding matrix
    for _ in range(n_new_bins):
        # Create a new row by following the multi-scale embedding logic
        new_row = torch.zeros(input_dim, device=device, dtype=dtype)
        scale = len(S_update) % scales + 1  # Determine which scale this row corresponds to
        new_row[(scale - 1) * step : scale * step] = torch.randn(
            step, device=device, dtype=dtype
        )

        # Append the new row to the embedding matrix
        S_update = torch.cat((S_update, new_row.unsqueeze(0)), dim=0)

        # Add a new column to the observations (initialized to zeros)
        new_column = torch.zeros(n_samples, 1, device=device, dtype=dtype)
        X_update = torch.cat((X_update, new_column), dim=1)

    # Debugging checks
    assert S_update.shape == (target_dim + n_new_bins, input_dim), (
        f"Updated embedding matrix has incorrect shape: {S_update.shape}"
    )
    assert X_update.shape == (n_samples, target_dim + n_new_bins), (
        f"Updated observations matrix has incorrect shape: {X_update.shape}"
    )
